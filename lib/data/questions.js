...
export const ALL_QUESTIONS = [...MATH_QUESTIONS, ...ENGLISH_QUESTIONS];
...
export function buildSubjectSet(subject, count = 10, seed = 7) {
  const rand = mulberry32(seed);
  return pick(questionsForSubject(subject), count, rand);
}

export function buildTopicSet(topicId, count = 6, seed = 11) {
  const rand = mulberry32(seed);
  return pick(questionsForTopic(topicId), count, rand);
}
