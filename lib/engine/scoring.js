...
/** Text answers: trim, lowercase, collapse spaces, drop trailing punctuation. */
const normText = (value) => String(value ?? "").trim().toLowerCase().replace(/\s+/g, " ").replace(/[.!?,;:]+$/g, "");

/** Math/equation answers: ignore spacing and unify operator spellings. */
const normMath = (value) =>
  String(value ?? "")
    .replace(/\s+/g, "")
    .toLowerCase()
    .replace(/×/g, "*")
...
const toBool = (value) => value === true || value === 1 || ["1", "true", "t", "yes"].includes(String(value).trim().toLowerCase());
...
export function isAnswerCorrect(question, given) {
  ...
  if (type === "boolean") {
    return toBool(given) === toBool(question.answer);
  }
  if (type === "fill" || type === "short") {
    const accepted = [question.answer, ...(question.acceptable ?? [])].map(normText);
    return accepted.includes(normText(given));
  }
  if (type === "equation") {
    const accepted = [question.answer, ...(question.acceptable ?? [])].map(normMath);
    return accepted.includes(normMath(given));
  }
  /* mcq, reading, vocabulary — the answer is the option index */
