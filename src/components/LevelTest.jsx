...
export default function LevelTest() {
    ...
    const handleExit = () => {
        if (stage === 'test' && !finished) {
            toast.custom(
                (toastItem) => (
                    <div className={`${toastItem.visible ? 'animate-enter' : 'animate-leave'} max-w-sm w-full bg-white dark:bg-gray-900 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-2xl pointer-events-auto flex flex-col gap-4 p-5 sm:p-6 border border-gray-100 dark:border-gray-800 font-sans`}>
    ...
    if (stage === 'intro') {
        return (
            <div className="min-h-screen flex items-center justify-center px-3 xs:px-4 pt-16 xs:pt-20 pb-8 xs:pb-12 transition-colors duration-200 relative overflow-hidden font-sans">
    ...
    if (finished && !showResult) {
        return (
            <div className="min-h-screen flex items-center justify-center px-3 xs:px-4 pt-16 xs:pt-20 pb-8 xs:pb-12 transition-colors duration-200 font-sans">
                <Toaster position="top-center" />
                <ExitBtn />
