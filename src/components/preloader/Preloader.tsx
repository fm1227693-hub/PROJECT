...
export default function Preloader({ onComplete }: PreloaderProps) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      exitTl.to(lGlyphRef.current, { scale: 1, opacity: 1, rotate: 0, duration: 0.9, ease: 'power4.out' }, 0.4)
      ...
      exitTl.to(curtainTopRef.current, { yPercent: -100, duration: 1.1, ease: 'power4.inOut' }, 1.5)
      exitTl.to(curtainBottomRef.current, { yPercent: 100, duration: 1.1, ease: 'power4.inOut' }, 1.5)
      ...
      <div ref={counterRef} className="absolute bottom-6 md:bottom-10 left-6 md:left-10 z-20 will-change-transform">
        <div className="flex items-baseline gap-4">
          ...
          <div className="hidden md:block text-white/30 font-mono text-[11px] tracking-[0.2em] leading-[1.4] max-w-[160px] -translate-y-6">
            LUSION®<br />EST. 2018<br />PARIS — TOKYO
          </div>
        </div>
      </div>
