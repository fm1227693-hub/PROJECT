...
export default function Page() {
  ...
  // Single Lenis instance — singleton prevents lag
  useLenisScrubDriver({
    lerp: 0.08,
    wheelMultiplier: 0.9,
    smoothWheel: true,
  ...
  useEffect(() => {
    ...
    const onScroll = (e: Event): void => {
      ...
      requestAnimationFrame(() => {
        ...
        if (custom.detail) {
          const progress = typeof custom.detail.progress === 'number' ? custom.detail.progress : 0
          setScrollProgress(progress)
          lastScroll.current = custom.detail.scroll ?? lastScroll.current
        }
        ticking = false
      })
    }

    window.addEventListener('lusion-scroll' as never, onScroll as EventListener)
    return () => window.removeEventListener('lusion-scroll' as never, onScroll as EventListener)
  }, [])
  ...
  return (
    <main className="relative bg-[#f7f7f9] text-[#0b0b0d] selection:bg-[#0b0b0d] selection:text-white overflow-x-hidden">
      ...
      {/* BLUE RIBBON 2D — Ultra lightweight, no RAF loop */}
      <BlueRibbon2D scrollProgress={scrollProgress} />

      {/* SHOWREEL 2D — Lightweight docking */}
      ...
      <div className="relative z-10 min-h-[92vh] flex flex-col justify-between px-6 md:px-8 lg:px-10 pt-[88px] pb-8 pointer-events-none">
        ...
        <div className="flex-1 flex items-center justify-center">
          ...
          >
            ...
            <button
              className={`pointer-events-auto group relative w-[64px] h-[64px] md:w-[84px] md:h-[84px] rounded-full bg-white text-black flex items-center justify-center text-[22px] md:text-[26px] shadow-[0_12px_36px_rgba(0,0,0,0.2)] transition-transform duration-300 cursor-pointer ${
                isHovering ? 'scale-[1.08]' : 'scale-100'
              }`}
            >
      ...
      <div className="relative z-10">
        ...
        {/* ASTRONAUT HYPERSPACE PORTAL — Ultra lightweight, no lag */}
        ...
        <footer className="relative z-10 bg-[#f7f7f9] border-t border-black/10 px-6 md:px-8 lg:px-10 py-10 flex flex-col md:flex-row justify-between gap-6 text-[11px] font-mono tracking-[0.15em] text-black/30">
          <div>©2026 LUSION® — ULTRA LIGHTWEIGHT • NO LAG • 60FPS • BLUE #2563eb</div>
      ...
      <div
        className="fixed bottom-6 left-6 md:bottom-8 md:left-8 z-[5] pointer-events-none transition-all duration-500"
        style={{
          opacity: dockProgress > 0.55 ? 1 : 0,
          transform: `translateY(${dockProgress > 0.55 ? 0 : 8}px)`,
        }}
      >
